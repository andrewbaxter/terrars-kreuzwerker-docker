...
impl Dependable for Config {
    fn extract_ref(&self) -> String {
        Resource::extract_ref(self)
    }
}

impl ToListMappable for Config {
    type O = ListRef<ConfigRef>;
...
pub struct BuildConfig {
    pub tf_id: String,
    #[doc= "Base64-url-safe-encoded config data"]
    pub data: PrimField<String>,
    #[doc= "User-defined name of the config"]
    pub name: PrimField<String>,
}

impl BuildConfig {
    pub fn build(self, stack: &mut Stack) -> Config {
        let out = Config(Rc::new(Config_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(ConfigData {
